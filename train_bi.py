...
def train_full(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer):
    encoder_hidden = encoder.initHidden(device)
    ...
    # enc_padding_mask = np.zeros((1, max_length), dtype=np.float32)
    # enc_padding_mask = Variable(torch.from_numpy(enc_padding_mask)).float()
    # for j in range(input_length):
    #     enc_padding_mask[0][j] = 1
    ...
    coverage = None
    if cfg.IS_COVERAGE:
        coverage = Variable(torch.zeros(input_length))
    ...
    if start_iter < n_iters:
        ...
        for i in range(start_iter, n_iters):
            ...
            if cfg.SIMPLE_ATTENTION:
                ...
                loss = train_full(input_tensor, target_tensor, encoder,
                             decoder, encoder_optimizer, decoder_optimizer)
